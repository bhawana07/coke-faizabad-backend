  ...
  if ((new Date() - last_update <= 15000 || inside) && multi_res) {
          ...
          pipeline: [
            ...
            },
            {
              $project: {
                ...
                schedule_break: "$machine_wise.schedule_break",
                machine_name: "$machine_wise.machine_name",
    ...
    if(data.length > 0){
      data.forEach(async (element, j) => {
        ...
        if (element.status && element.status.last_update && element.shift_batch_wise_array) {
          ...
          var batch_cal_data = calculation(
            ...
            element.status.batch_wise_cip_duration || 0,
            element.schedule_break || 0
          );
          var shift_cal_data = Shiftcalculation(
            ...
            element.shift_batch_wise_array.npt,
            element.shift_batch_wise_array.et,
            element.shift_batch_wise_array.cip,
            element.shift_batch_wise_array.changeover
          );
          var compare_obj = {
            ...
            me:batch_cal_data.me,
            sle:batch_cal_data.sle,
          ...
          push_obj.shift_cip = convertHHMM(
            element.shift_batch_wise_array.cip
          );
          ...
          push_obj.shift_npt =  convertHHMM(element.shift_batch_wise_array.npt);
          push_obj.shift_et =  convertHHMM(element.shift_batch_wise_array.et);
          push_obj.shift_sle = shift_cal_data.sle;
          push_obj.shift_me = shift_cal_data.me;
          ...
          push_obj.batch = element.batch.manual_batch_name ? element.batch.manual_batch_name : element.batch.batch;
          ...
          delete element.fgex.updateArr; 
          push_obj["batch_obj"] = element.batch;
          ...
          push_obj["batch_cip"] = `${
            element.status.batch_wise_cip_count || 0
          } / ${convertHHMM(
            element.status.batch_wise_cip_duration || 0
          )}`;
          ...
          push_obj["batch_sle"] = await getColour(
            compare_obj,
            "sle",
            batch_cal_data.sle,
            " %"
          );
          push_obj["batch_me"] = await getColour(
            compare_obj,
            "me",
            batch_cal_data.me,
            " %"
          ...
          push_obj["batch_et"] = convertHHMM(batch_cal_data.et);
          push_obj["batch_npt"] = convertHHMM(batch_cal_data.npt);
          ...
          push_obj["machine"] = {
            ...
            isCritical:
              ...
              isLastMachine : element.last_machine_count_machine == element.machine_wise.machine_name,
          };
...
async function getLiveChartData(line_id, cb) {
  ...
  add15minCache(line_id, (x, y, data) => {
    var line_arr = data.filter((a)=>String(a.line_id) == String(line_id));
    var critical_mach = line_arr.find((mac) => {
      return (addLines.critical_machine = mac.equipment.equipment_name);
    });

    var last_machine_data = line_arr.find((mac) => {
    ...
    batch_wise_arr.forEach((element) => {
      var push_obj = {
        ...
        last_machine_goodcount:
          last_machine_data.shift_batch_wise_array.raw_good_count,
        ...
        batch_name: element.batch.manual_batch_name ?  element.batch.manual_batch_name : element.batch.batch,
...
async function getLiveChartDataAllMachine(line_id, cb) {
  try {
    const tempData = await TempGood.findOne({ line_id: line_id })
      .populate({ path: "currnt_batch", populate: { path: "product_name" } })
      .populate({ path: "vendor", populate: { path: "vendor" } });

    const send_data = [];
    add15minCache(line_id, async (x, y, data) => {
      const line_arr = data.filter((a) => String(a.line_id) === String(line_id));

      for (const element of line_arr) {
        const shift_data = { ...element.shift_batch_wise_array };
        delete shift_data.batch_wise;

        shift_data.fgex = {
          sku_number: tempData.currnt_batch.product_name.sku_number,
          sku_description: tempData.currnt_batch.product_name.sku_description,
          rated_speed: tempData.currnt_batch.product_name.rated_speed,
          bottles_per_case: tempData.currnt_batch.product_name.bottles_per_case,
        };

        shift_data.vendor = tempData.vendor.vendor.vendor;
        shift_data.batch = tempData.currnt_batch.batch;
        shift_data.shift = tempData.current_shift;
        shift_data.date = tempData.date;
        shift_data.operator_name = tempData.current_operator;
...
function calculation(
  ...
  var ppt_time = total_batch_time - pdt - updt - cip - changeover;
  var got_time = ppt_time - major_fault - major_manual_stop;
  var npt = total_batch_time - schedule_break - pdt;
  var et = total_count / (rated_speed / 60);
  if (npt < 70) {
    npt = 0;
  }
  ...
  data.oee = me;//data.quality * data.aviability * data.performance;
